...
- **☁️ Serverless 架构**：依托于 Vercel Serverless API，试卷数据可以轻松持久化存储，无需部署复杂的传统后端服务。

- **🔒 数据安全性**：告别超长 URL 传参。可视化创建题目后，系统将数据安全地存入云端数据库，并生成简短唯一的动态分享链接。
...
1. 在首页右上角点击 **“我也要出题”** 按钮（自带精美的克莱因蓝主题动效过渡）。
2. 输入你的专属身份名称和用于后台对答案的“密语”。
3. 每道题目自主填空，或从内置题库中一键选取，同时设定你最真实的“自定评分”。
4. 完成所有评判后点击发布，前端会自动提交到 Serverless 云端接口，并生成一张充满设计感的分享海报（包含直达测验的精准二维码）。
5. 长按或点击保存网页生成的精美二维码卡片分享给朋友。

---

## 🛠️ 核心实现原理

随着全新功能的加入，本作升级为了**纯前端渲染 + Serverless 数据接口**的全栈构架体验：

- **数据持久化**：出题人在前端零代码配好属于自己的题目后，应用调用 Vercel 的 Serverless Function（以 Node.js 等运行时触发），将试卷 JSON 数据存储至 Vercel 内置云端数据库（或诸如 Firebase/Supabase/KV 等灵活存储方案），并返回全局唯一的 `userid`。
- **动态解析**：当朋友扫码/打开由 `userid` 拼成的链接时，项目发起获取请求动态拉取题库数据并完美渲染。不仅短链美观，且打破了原本 URL 长度对问题数量的硬件级约束。
